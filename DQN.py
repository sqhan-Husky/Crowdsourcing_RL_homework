...
class DQN():
    def __init__(self, seq_len, histo_len,category, n_actions, batch_size, lr, epsilon, gamma, target_replace_iter, memory_capacity):
        ...
        self.n_layers = 5
        self.histo_len = 7
        self.eval_net = Net(self.max_pj, self.embedding_dim, self.seq_len, self.n_actions, self.histo_len,self.rnn_hidden_dim,self.n_layers)
        self.target_net = Net(self.max_pj, self.embedding_dim, self.seq_len, self.n_actions, self.histo_len,self.rnn_hidden_dim,self.n_layers)
    ...
    def learn(self):
        ...
        b_state = torch.LongTensor([b[: self.seq_len+self.histo_len] for b in b_memory])
        b_action = torch.LongTensor([b[self.seq_len+self.histo_len: self.seq_len+self.histo_len + 1] for b in b_memory])
        b_reward = torch.FloatTensor([b[self.seq_len+self.histo_len+1: self.seq_len+self.histo_len + 2] for b in b_memory])
        b_state_next = torch.LongTensor([b[-(self.seq_len+self.histo_len):] for b in b_memory])
