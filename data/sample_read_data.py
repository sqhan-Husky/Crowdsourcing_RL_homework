...
for line in project_list_lines:
    ...
    k = 0
    # while (k < entry_count):
    #     file = open(entry_dir + "entry_" + str(project_id) + "_" + str(k) + ".txt", "rb")
    #     htmlcode = file.read()
    #     file.close()
    #     text = json.loads(htmlcode)
    #
...
entry_info = pd.DataFrame(entry_info)
project_info.to_csv(project_path)
entry_info.to_csv(entry_path)

# project_info = json.dumps(project_info)
# entry_info = json.dumps(entry_info)

# f1 = open(project_path, 'w')
# f1.write(project_info)
# f1.close()
#
# f2 = open(entry_path, 'w')
# f2.write(entry_info)
# f2.close()
